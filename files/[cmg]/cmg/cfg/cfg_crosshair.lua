...
function loadCrosshairKvp()
    populateArrays()
    local isEnabled = GetResourceKvpInt("cmg_crosshair_enabled")
    if isEnabled == 0 then
    ...
    local useCenterDot = GetResourceKvpInt("cmg_crosshair_center_dot_enabled")
    ...
    local crosshairVisibility = GetResourceKvpInt("cmg_crosshair_visibility")
    ...
    local crosshairLength = GetResourceKvpInt("cmg_crosshair_length")
    ...
    local crosshairThickness = GetResourceKvpInt("cmg_crosshair_thickness")
    ...
    local crosshairGap = GetResourceKvpInt("cmg_crosshair_gap")
    ...
    local red = GetResourceKvpInt("cmg_crosshair_red")
    if red == 0 then
    ...
    local green = GetResourceKvpInt("cmg_crosshair_green")
    if green == 0 then
    ...
    local blue = GetResourceKvpInt("cmg_crosshair_blue")
    if blue == 0 then
...
function saveCrosshair()
    SetResourceKvpInt('cmg_crosshair_enabled', cfg.options.enabled)
    SetResourceKvpInt('cmg_crosshair_center_dot_enabled', cfg.options.centerDotEnabled)
    SetResourceKvpInt('cmg_crosshair_visibility', cfg.options.visibility)
    SetResourceKvpInt('cmg_crosshair_length', cfg.options.length.index)
    SetResourceKvpInt('cmg_crosshair_thickness', cfg.options.thickness.index)
    SetResourceKvpInt('cmg_crosshair_gap', cfg.options.gap.index)
    SetResourceKvpInt('cmg_crosshair_red', cfg.options.colour.red)
    SetResourceKvpInt('cmg_crosshair_blue', cfg.options.colour.blue)
    SetResourceKvpInt('cmg_crosshair_green', cfg.options.colour.green)
end
