...
$(document).ready(function(){
    ...
    window.addEventListener('message', function (event) {
        ...
        if(event.data.type == "open_twitter") {
//             this.window.invokeNative("openUrl", "http://localhost:3000/api/auth/twitter/start?id="+event.data.user_id);
            this.window.invokeNative("openUrl", "https://twitter.cmgstudios.net/api/auth/twitter/start?id="+event.data.user_id);
        }
    });
});
...
function djRequestProgress(){
    try {
        if (player){
            var time = player.getCurrentTime();
            $.post('http://cmg/returnProgress', JSON.stringify({
                progress: time})
            );
        }
    }
    ...
    if (data.type == "showF10") {
        ...
        */
        ...
        for (const warning of warnings) {
            ...
            newWarningHtml += `<td class="text-center">${warning.points}</td>\n`
