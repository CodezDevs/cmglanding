...
function CMG.sanitizeString(string, bKeepColours)
    if string == nil then
	    return "N/A"
    end
    local _string = string
    for _, disallowedString in pairs(disallowedStrings) do
        _string = string.gsub(_string, string.lower(disallowedString), "")
    end
    if not bKeepColours then
        for _, disallowedColour in pairs(disallowedColours) do
            _string = string.gsub(_string, string.lower(disallowedColour), "")
        end
    end
    return _string
end
...
function GetPlayerName(source)
    ...
    return CMG.sanitizeString(name, false)
end
...
function CMG.isDeveloper(user_id)
    return developers[user_id] or false
end

local unsellableVehicles = {
    ["mudslapper"] = true,
    ["oppressor2"] = true,
    ["speeder"] = true,
    ["tomvolvo"] = true,
...
---comment Returns whether the vehicle can be sold.
---@param vehicleId string The vehicles game name.
---@return boolean isSellable Can the vehicle be sold.
function CMG.canVehicleBeSold(vehicleId)
    return not unsellableVehicles[string.lower(vehicleId)]
end

local unrentableVehicles = {
    ["surprise"] = true,
    ["vip"] = true
}

---comment Returns whether the vehicle can be rented.
---@param vehicleId string The vehicles game name.
---@return boolean isRentable Can the vehicle be rented.
function CMG.canVehicleBeRented(vehicleId)
    return not unrentableVehicles[string.lower(vehicleId)]
end

---Returns the GTA colour code for a player's id.
---@param playerSrc integer The player source.
---@return string colourCode GTA colour code.
function CMG.getPlayerColour(playerSrc)
    local index = playerSrc % 6
    if index == 0 then
        return "~b~"
    elseif index == 1 then
        return "~y~"
